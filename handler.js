...
export async function participantsUpdate({ id, participants, action }) {
    ...
    mediaUrl: "https://instagram.com/Fdev90",
    mediaType: 2, 
    description: "https://youtu.be/-TleC8rDT8", 
    title: 'RezBotz~レゼ',
    body: wm,
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (!text) continue
         this.send2ButtonDoc(id, text.trim(), author, 'OFF', '.off detect', 'MENU', '.menu', fakes, adReply)
    }
}
...
*.delete*`, author, 'OFF', '.on delete', 'MENU', '.menu', msg, adReply)
...
global.dfail = (type, m, conn) => {
    ...
    if (msg) return conn.send2ButtonDoc(m.chat, msg, author, 'OWNER', '.creator', 'MENU', '.menu', fpayment, adReply)
}
